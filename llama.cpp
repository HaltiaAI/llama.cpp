...
#define MLOCK_SUGGESTION "Try increasing RLIMIT_MLOCK ('ulimit -l' as root).\n"
  ...
  bool raw_lock(const void *addr, size_t size) const {
    ...
    LLAMA_LOG_WARN("warning: failed to mlock %zu-byte buffer (after previously "
                   "locking %zu bytes): %s\n%s",
...
#undef MLOCK_SUGGESTION
  ...
  bool raw_lock(void *ptr, size_t len) const {
    for (int tries = 1;; tries++) {
      ...
      if (tries == 2) {
        LLAMA_LOG_WARN("warning: failed to VirtualLock %zu-byte buffer (after "
    ...
    case LLM_ARCH_LLAMA: {
      ...
      // output
      {
        ...
        model.output_norm = ml.create_tensor(
            ctx, tn(LLM_TENSOR_OUTPUT_NORM, "weight"), {n_embd}, backend_norm);
    ...
    case LLM_ARCH_FALCON: {
      ...
      for (uint32_t i = 0; i < n_layer; ++i) {
        ...
        if (gguf_find_tensor(ml.ctx_gguf,
                             tn(LLM_TENSOR_ATTN_NORM_2, "weight", i).c_str()) >=
            0) {
        ...
        layer.wqkv =
            ml.create_tensor(ctx, tn(LLM_TENSOR_ATTN_QKV, "weight", i),
...
static bool llama_model_load(const std::string &fname, llama_model &model,
  ...
  try {
    ...
    llm_load_tensors(*ml, model, n_batch, n_gpu_layers, main_gpu, tensor_split,
                     mul_mat_q, low_vram, memory_type, use_mlock,
...
static struct ggml_cgraph *llm_build_falcon(llama_context &lctx,
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    // TODO: refactor into common function (shared with LLaMA)
    {
      ...
      attn_norm =
          ggml_add(ctx0, ggml_mul(ctx0, attn_norm, model.layers[il].attn_norm),
      ...
      // TODO: these 2 ggml_conts are technically not needed, but we add them
      // until CUDA support for
      //       non-contiguous views is added for the rope operator
      struct ggml_tensor *tmpq = ggml_cont(
          ctx0,
          ggml_view_3d(ctx0, cur, n_embd_head, n_head, N, wsize * n_embd_head,
                       wsize * n_embd_head * (n_head + 2 * n_head_kv), 0));
      offload_func_kq(tmpq);

      struct ggml_tensor *tmpk = ggml_cont(
          ctx0, ggml_view_3d(ctx0, cur, n_embd_head, n_head_kv, N,
                             wsize * n_embd_head,
          ...
          for (auto j = str.begin(); j != str.end(); ++j) {
            ...
            if (token_multibyte == vocab.token_to_id.end()) {
              try {
                ...
                LLAMA_LOG_ERROR("ERROR: byte not found in vocab: '%s'\n",
                                byte_str.c_str());
              }
            } else {
...
llama_tokenize_internal(const llama_vocab &vocab, std::string raw_text,
  ...
  case LLAMA_VOCAB_TYPE_SPM: {
    // without adding this leading whitespace, we do not get the same results as
...
static void llama_grammar_advance_stack(
  ...
  case LLAMA_GRETYPE_CHAR_NOT:
    ...
    break;
  default:
    // end of alternate (LLAMA_GRETYPE_END, LLAMA_GRETYPE_ALT) or middle of char
    // range (LLAMA_GRETYPE_CHAR_ALT, LLAMA_GRETYPE_CHAR_RNG_UPPER); stack
    // should never be left on those
    GGML_ASSERT(false);
  }
}
...
struct llama_beam_search_data {
  ...
  void loop(const llama_beam_search_callback_fn_t callback,
    ...
    for (int i = 0;
      ...
      update_beams_from_beam_views(); // Update values (p,eob) that callback may
                                      // have changed.
  ...
  while (true) {
    ...
    if (lora_tensors.find(base_name + ".loraA") != lora_tensors.end() &&
      ...
      if (ggml_is_quantized(base_t->type)) {
        if (!warned) {
          LLAMA_LOG_WARN("%s: warning: using a lora adapter with a quantized "
...
llama_load_model_from_file(const char *path_model,
  ...
  if (!llama_model_load(
          path_model, *model, params.n_ctx, params.n_batch, params.n_gpu_layers,
